...
def get_farms():
    res = supabase.table("farms").select("*").order("created_at", desc=True).limit(6).execute()
    return jsonify(res.data)
...
def calculate(project_id):
    try:
        ...
        if not p: return jsonify({"error": "No project"}), 404
        ...
        return jsonify({
            ...
            "emission_kg": round(total_e, 1), "sink_kg": round(total_s, 1), "net_impact": round(total_e - total_s, 1)
        })
...
def add_farm():
    ...
    # 解決日期為空導致 500 的關鍵
    in_d = f.get('in_date') if f.get('in_date') else None
    out_d = f.get('out_date') if f.get('out_date') else None

    supabase.table("farms").insert({
        "batch_number": f.get('batch_number'), "plant_name": f.get('plant_name'),
        "quantity": int(f.get('quantity', 0)), "in_stock_date": in_d,
        "out_stock_date": out_d, "photo_url": p_url
    }).execute()
